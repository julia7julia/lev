...
Этапы работы:
Предварительная подготовка: Если необходимо, установите ansible 
...
В этот каталог скопировать присланные файлы Vagrant
...
Используя эти параметры, создадим свой первый inventory файл с помощью команды nano inventory или в сразу добавим в Visual Studio.
Записываем в этот файл данные
cat inventory
Данные для записи:
[webservers]
nginx ansible_host=127.0.0.1 ansible_port=2222 ansible_private_key_file=/home/mary/os_lab2/test/ansible/.vagrant/machines/nginx/virtualbox/private_key
...
Чтобы не пришлось в дальнейшем каждый раз явно указывать наш инвентори файл в
...
Теперь, когда мы убедились, что у нас все подготовлено - установлен Ansible, поднят хост для
теста и Ansible имеет к нему доступ, мы можем конфигурировать наш хост.
Для начала воспользуемся Ad-Hoc командами и выполним некоторые удаленные команды
на нашем хосте.
...
3. Напишем плейбук, который устанавливает NGINX в конфигурации по умолчанию, с
применением модуля yum

Создаем Playbook, который будет выполнять установку пакета epel-release. 
Создаем файл epel.yml со следующим содержимым:
---
...
За основу возьмем уже созданный плейбук epel.yml. Скопируем этот файл с именем nginx.yml.
...
Обратите внимание - добавили tags. Теперь можно вывести в консоль список тегов и
...
playbook: nginx.yml
  ...
  play #1 (webservers): Install EPEL Repo       TAGS: []
...
5. Подготовим шаблон jinja2 новой конфигурации для nginx, чтобы сервис слушал на
нестандартном порту 8080. В шаблоне для номера порта использовать переменные
ansible

Создадим файл шаблона для конфига NGINX, имя файла nginx.conf.j2. Обратите
внимание, что в шаблоне используется переменная, которую в дальнейшем надо где-то
определить.
Данные для записи:
events {
...
Добавим в playbook задачу, которая копирует подготовленный шаблон на хост. 
Также в плейбук добавлено определение переменной в секции vars.
Для этого в nginx.yml введем следующее:
---
- name: Install EPEL Repo
  hosts: webservers
  become: true
...
Добавить секции handler и notify для рестарта nginx не при любом старте playbook, а только при изменения в конфигурации. 
Для этого в nginx.yml введем следующее:
---
- name: Install EPEL Repo
  hosts: webservers
...
Чтобы проверить работу NGINX на нестандартном порту, нам надо выяснить IP
...
3: eth1: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc pfifo_fast state UP group default qlen 1000
    link/ether 08:00:27:1c:63:58 brd ff:ff:ff:ff:ff:ff
    inet 192.168.11.150/24 brd 192.168.11.255 scope global noprefixroute eth1
    ...)

В полученном выводе найдите описание сетевых интерфейсов. Первый интерфейс —
...
6. Затем в браузере открываем страницу:
